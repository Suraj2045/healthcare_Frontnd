...
const UpdateProcedureArea = ({ procedure }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8004/providerprofiles/${procedure.id}`
        );
  ...
  //    await fetch(`http://localhost:8081/procedures/update/${id}`, {
  //       method: "PUT",
  //       headers: {
  ...
  //    fetch(`http://localhost:8081/procedures/${id}` , {
  //      method: 'PUT',
  //      headers: {
  //        'Content-Type': 'application/json',
  //      },
  ...
  const handleProcedureRegistration = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8004/Procedure/update/${id}`,
        formData,
        {
          headers: {
